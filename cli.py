...
def dev_command(task: str, verbose: bool = True) -> None:
    ...
    from council.protocol.schema import DevStatus
    from council.observability.logger import get_logger
    ...
    # 运行编排器
    print("\n") # Start streaming on new line
    state = asyncio.run(orchestrator.dev(task, stream_handler=stream_handler))
    print("\n") # End streaming

    # 输出结果
    if HAS_RICH and console:
        if state.status == DevStatus.COMPLETED:
            console.print("\n[bold green]✅ 完成![/bold green]")
        elif state.status == DevStatus.FAILED:
            console.print("\n[bold red]❌ 失败[/bold red]")
        else:
            console.print(f"\n[bold yellow]⚠️ {state.status.value}[/bold yellow]")

        console.print(f"[dim]状态: {state.status.value}[/dim]")
