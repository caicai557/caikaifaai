            ...
            with open(LEDGER_FILE, 'r') as f:
                ...
                for task in data.get("tasks", []):
                    db.execute(
                        ...
                        (task["id"], task["description"], task["status"], task["agent"], task.get("confidence", 0.0), task.get("created_at"), task.get("updated_at"))
                    )
            print(f"✅ Loaded {len(data.get('tasks', []))} tasks from disk into memory.", file=sys.stderr)
...
@mcp.tool()
def update_task(task_id: str, status: Optional[str] = None, confidence: Optional[float] = None, agent_id: str = "claude") -> str:
